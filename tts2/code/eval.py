import math
files = ["./data/systems/S1.results","./data/systems/S2.results","./data/systems/S3.results","./data/systems/S4.results","./data/systems/S5.results","./data/systems/S6.results"]

def read_file(title,results):
    with open(title, encoding="utf8", errors='ignore') as f:
        file = int(title[16:].split(".")[0]) 
        # file is 1,2...
        results[file] = {}
...
def read_true_results():
    true = {}
    with open("./data/systems/qrels.txt", encoding="utf8", errors='ignore') as f:
        for line in f.readlines():
            line = line.split(" ")
            q_id = line[0][:-1]
            if q_id not in true:
